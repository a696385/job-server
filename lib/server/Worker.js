...
Worker.prototype._onMessage = function(data, flags){
	...
	if (msg.type === "get-jobs"){
		...
		if (this.isBloked || this.workedCount >= this.maxqps){
			sendError(msg.id, Errors.E3002());
			return;
		}
		this.server.store.takeJob(msg.data.jobId, self.id, function(err, job, needBlock){
			if (!err){
				self.workedCount++;
				if (needBlock) self.isBloked = true;
				sendSuccess(msg.id);
			} else {
