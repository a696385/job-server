				...
				if (msg.data.workerId != null){
					for(var i = 0, len = self.server.workers.length; i < len; i++){
						if (self.server.workers[i].id === msg.data.workerId){
	                        (function(worker){
	                            worker.isBloked = true;
	                            worker.allowTake = jobId;
	                            setTimeout(function(){
	                                if (worker.isBloked) return;
	                                self.server.store.getWorkerCurrentJobs(worker.id, function(err, result){
	                                    if (err) return;
	                                    if (result.length === 0) worker.isBloked = false;
	                                });
	                            }, 1000 * 60);
	                        })(self.server.workers[i]);
							break;
						}
					}
				}
...
Worker.prototype._onJobComplete = function(jobId){
	...
	this.server.store.getJob(jobId, function(err, job){
		...
		var workerCompletedJob = self.server.getWorker(job.workerId);
		if (workerCompletedJob !=  null && msgInfo.unblock){
			workerCompletedJob.isBloked = false;
			workerCompletedJob.allowTake = null;
		}
        self.server.store.removeJob(jobId);
	});
};
...
Worker.prototype._notificationForJobs = function(){
	...
	self.server.store.getJobs(self.id, function(err, jobs){
		...
		jobs.forEach(function(job){
			self.server.workers.forEach(function(worker){
				worker._onJobCreate(job.id);
			});
		});
	});
};
